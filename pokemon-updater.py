#!/usr/bin/env python3
"""Build and publish the Cardly database files from a pluggable source adapter."""
...
def load_source_module(path: Path) -> ModuleType:
    ...
    missing = [name for name in SOURCE_ADAPTER_FUNCTIONS if not callable(getattr(module, name, None))]
    if missing:
        raise RuntimeError(
            f"Source adapter {path} is missing required functions: {', '.join(missing)}"
        )
    return module
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-module",
        type=Path,
        default=Path(__file__).with_name("sources") / "tcgdex.py",
        help="Path to a source adapter (default: sources/tcgdex.py)",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest = publish_manifest(
        ...
        regions,
        args.tcgdex_release,
        update_type="prices" if args.prices_only else "full",
    )
    print(
