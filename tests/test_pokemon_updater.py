...
class FakeSource:
    ...
    def transform_price_data(self, card_id, card_data):
        assert card_data["name"] == "Pikachu"
        return [{"card_id": card_id, "variant_id": "variant-1", "market_source": "fake", "price_type": "normal", "updated_at": "changes"}]


class CatalogExportTests(unittest.TestCase):
    def test_source_adapter_contract_is_validated(self):
        with tempfile.TemporaryDirectory() as temporary:
            source_path = Path(temporary) / "incomplete.py"
            source_path.write_text("VALUE = 1\n", encoding="utf-8")

            with self.assertRaisesRegex(RuntimeError, "missing required functions"):
    ...
    def test_export_writes_bulk_files_and_stable_manifest(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertNotIn("set_name", cards[0])
    ...
    def test_price_only_export_preserves_cards_sets_and_release(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest = MODULE.publish_manifest(root, [region], update_type="prices")
            ...
            self.assertEqual(manifest["updateType"], "prices")
            self.assertEqual(manifest["regions"]["international"]["priceCount"], 1)
